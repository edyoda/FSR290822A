...
$(document).ready(function () {
    ...
    for (var i = 0; i < playlistData.length; i++) {
        var x = createPlaylistCard(playlistData[i]);
        $('#playlist-wrapper').append(x);
    }

    var card = document.getElementsByClassName("playlist-card");

    for (var i = 0; i < card.length; i++) {
        if (i == 0) {
            card[0].classList.add("active-card");
        }

        card[i].addEventListener('click', function () {
            var activeCard = document.getElementsByClassName("active-card");

            if (activeCard.length != 0) {
                activeCard[0].classList.remove("active-card");
            }

            //this will refer to card
            this.className = "active-card";

            console.log(this.id.slice(4));

            for (var k = 0; k < videoPlaySectionData.length; k++) {
                if (k == this.id.slice(4)) {
                    $("#video-title").html(videoPlaySectionData[k - 1].title);
                    $("#video-description").html(videoPlaySectionData[k - 1].description);
                    $("#video-player").attr("src", playlistData[k - 1].thumbnail);
                }
            }
        })
